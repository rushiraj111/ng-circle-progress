...
export class CircleProgressComponent implements OnChanges, OnInit, OnDestroy {
  ...
  draw = (percent: number) => {
    ...
    let endPoint = this.polarToCartesian(
      ...
      centre.y,
      this.options.radius,
      (360 * (this.options.clockwise ? circlePercent : 100 - circlePercent)) /
        100
    ); // ####################
  ...
  getAnimationParameters = (
    ...
    let fromPercent = this.options.startFromZero
      ? 0
      : previousPercent < 0
      ? 0
      : previousPercent;
    let toPercent =
      currentPercent < 0
        ? 0
        : this.min(currentPercent, this.options.maxPercent);
  ...
  private applyOptions = () => {
    ...
    this.options.percent =
      +this.options.percent > 0 ? +this.options.percent : 0;
  ...
  checkViewport = () => {
    ...
    if (
      previousValue !== this.isInViewport &&
